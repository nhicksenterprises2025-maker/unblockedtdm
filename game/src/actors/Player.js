...
export class Player {
  constructor(spawn, team='blue', id='local-player') {
    ...
    this.health=new HealthState(); this.weaponManager=null;
    ...
    this.invulnerabilityTimer=0; this.dashDeniedTimer=0; this.dashStartedThisFrame=false; this.dashEndedThisFrame=false;
    this.velocityX=0; this.velocityY=0; this.trailTimer=0; this.dashGhostTimer=0; this.trail=[];
  }

  setWeaponManager(manager){ this.weaponManager=manager; }
  ...
  update(dt,input,map,camera) {
    ...
    if(!this.health.alive){
      this.sprinting=false; this.dashing=false; this.state='dead'; this.velocityX=0; this.velocityY=0;
      this.updateVisualMotion(dt); this.updateTrail(dt); return;
    }
  ...
  updateAim(input,camera){
    const pointer=input.pointerPosition(); const world=camera.screenToWorld(pointer.x,pointer.y);
    const dx=world.x-this.x, dy=world.y-this.y; if(Math.abs(dx)+Math.abs(dy)>0.001) this.aimAngle=Math.atan2(dy,dx);
  }

  updateLocomotion(dt,input,map){
    const axis=input.axis(); const moving=axis.x!==0||axis.y!==0;
    const wantsSprint=input.sprintHeld()&&!input.adsHeld()&&moving&&this.stamina>0;
    this.sprinting=wantsSprint;
    if(this.sprinting){
      this.stamina=Math.max(0,this.stamina-SPRINT_DRAIN_PER_SECOND*dt); this.staminaRegenDelay=SPRINT_REGEN_DELAY;
      if(this.stamina<=0) this.sprinting=false;
    } else this.updateStaminaRegen(dt);
    ...
    const speed=speedTiles*TILE_SIZE; const dx=axis.x*speed*dt, dy=axis.y*speed*dt;
    const beforeX=this.x,beforeY=this.y;
    moveCircle(this,dx,dy,map.blockers,{w:WORLD_WIDTH,h:WORLD_HEIGHT});
    this.velocityX=dt>0?(this.x-beforeX)/dt:0; this.velocityY=dt>0?(this.y-beforeY)/dt:0;
  ...
  updateDash(dt,map){
    ...
    const dx=Math.cos(this.dashDirection)*desired, dy=Math.sin(this.dashDirection)*desired;
    const bx=this.x,by=this.y;
    const result=moveCircleSwept(this,dx,dy,map.blockers,{w:WORLD_WIDTH,h:WORLD_HEIGHT},DASH_SWEEP_STEP_PIXELS);
    this.velocityX=dt>0?(this.x-bx)/dt:0; this.velocityY=dt>0?(this.y-by)/dt:0; this.moveAngle=this.dashDirection;
  ...
  updateVisualMotion(dt){
    const speedTiles=this.speedTilesPerSecond(), alive=this.health.alive;
    const movingTarget=alive&&speedTiles>0.05?1:0, sprintTarget=alive&&this.sprinting?1:0, dashTarget=alive&&this.dashing?1:0;
    this.motionBlend=approach(this.motionBlend,movingTarget,movingTarget?15:10,dt);
    this.sprintBlend=approach(this.sprintBlend,sprintTarget,11,dt); this.dashBlend=approach(this.dashBlend,dashTarget,dashTarget?24:13,dt);
    this.visualAimAngle=approachAngle(this.visualAimAngle,this.aimAngle,18,dt);
    const lower=speedTiles>0.08?this.moveAngle:this.aimAngle; this.visualMoveAngle=approachAngle(this.visualMoveAngle,lower,speedTiles>0.08?13:5,dt);
    const leanTarget=this.dashing?1:this.sprinting?0.55:speedTiles>0.08?0.16:0; this.bodyLean=approach(this.bodyLean,leanTarget,12,dt);
    const rate=this.dashing?2.1:this.sprinting?SPRINT_CYCLE_RATE:speedTiles>0.08?WALK_CYCLE_RATE:IDLE_CYCLE_RATE;
    this.animationTime+=dt; this.animationPhase=(this.animationPhase+dt*rate)%1;
  }

  updateTrail(dt){
    for(const ghost of this.trail)ghost.life-=dt; this.trail=this.trail.filter(g=>g.life>0); if(!this.health.alive)return;
    if(this.dashing){this.dashGhostTimer-=dt;if(this.dashGhostTimer<=0){this.dashGhostTimer=DASH_GHOST_INTERVAL;this.pushGhost('dash',DASH_GHOST_LIFETIME);}return;}
    if(!this.sprinting||this.speedTilesPerSecond()<0.05){this.trailTimer=0;return;}
    this.trailTimer-=dt;if(this.trailTimer<=0){this.trailTimer=TRAIL_INTERVAL;this.pushGhost('sprint',TRAIL_LIFETIME);}
  }
  ...
  respawn(spawn){
    this.x=spawn.x;this.y=spawn.y;this.stamina=SPRINT_STAMINA_MAX;this.staminaRegenDelay=0;this.sprinting=false;this.dashing=false;
    this.dashDistanceRemaining=0;this.dashCooldown=0;this.invulnerabilityTimer=0;this.velocityX=0;this.velocityY=0;this.state='idle';this.trail.length=0;
    this.health.respawn(); this.weaponManager?.resetForLife();
  }

  resetForRound(spawn=null){
    if(spawn){this.x=spawn.x;this.y=spawn.y;} this.stamina=SPRINT_STAMINA_MAX;this.staminaRegenDelay=0;this.dashCharges=DASH_CHARGES_MAX;
    this.dashing=false;this.dashDistanceRemaining=0;this.dashCooldown=0;this.invulnerabilityTimer=0;this.velocityX=0;this.velocityY=0;this.trail.length=0;
    this.health.resetForRound(); this.weaponManager?.resetForLife();
  }
