...
export class Player {
  ...
  updateLocomotion(dt,input,map,axis=input.axis(),blockers=map.blockers){
    ...
    const equipmentMultiplier=this.weaponManager?.movementMultiplier(input)??1;const speedTiles=PLAYER_SPEED_TILES*(this.sprinting?SPRINT_SPEED_MULTIPLIER:1)*equipmentMultiplier;const speed=speedTiles*TILE_SIZE;const dx=axis.x*speed*dt,dy=axis.y*speed*dt;const beforeX=this.x,beforeY=this.y;
    moveCircle(this,dx,dy,blockers,{w:WORLD_WIDTH,h:WORLD_HEIGHT});this.velocityX=dt>0?(this.x-beforeX)/dt:0;this.velocityY=dt>0?(this.y-beforeY)/dt:0;const actuallyMoving=Math.hypot(this.velocityX,this.velocityY)>.01;if(actuallyMoving)this.moveAngle=Math.atan2(this.velocityY,this.velocityX);this.state=actuallyMoving?(this.sprinting?'sprint':'walk'):'idle';
  }
  updateStaminaRegen(dt){if(this.staminaRegenDelay>0)this.staminaRegenDelay=Math.max(0,this.staminaRegenDelay-dt);else this.stamina=Math.min(SPRINT_STAMINA_MAX,this.stamina+SPRINT_REGEN_PER_SECOND*dt);}
  tryStartDash(direction=this.aimAngle){
    if(!this.health.alive||this.dashCooldown>0||this.dashCharges<=0||this.stamina<DASH_STAMINA_COST){this.dashDeniedTimer=.18;return false;}this.weaponManager?.cancelReload();this.dashing=true;this.sprinting=false;this.state='dash';this.dashDirection=direction;this.dashDistanceRemaining=DASH_DISTANCE_TILES*TILE_SIZE;this.dashCharges-=1;this.stamina=Math.max(0,this.stamina-DASH_STAMINA_COST);this.staminaRegenDelay=SPRINT_REGEN_DELAY;this.invulnerabilityTimer=DASH_INVULNERABILITY;this.dashStartedThisFrame=true;this.dashGhostTimer=0;return true;
  }
