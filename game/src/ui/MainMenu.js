...
function weaponCatalogCard(weapon, index) {
  ...
  return `
    <article class="ui231-weapon-card" data-catalog-weapon="${weapon.id}">
      <header class="ui231-weapon-card-head">
        <div>
          <span class="ui231-weapon-index">${String(index + 1).padStart(2, '0')} · ${slot}</span>
          <h3>${weapon.name}</h3>
          <small>${weaponRole(weapon)} · ${weapon.kind.toUpperCase()} · SWAP T${weapon.swapTier}</small>
        </div>
        <b>${weapon.shortName}</b>
      </header>
      <div class="ui231-weapon-model">${weaponModelSvg(weapon)}</div>
      <p class="ui231-weapon-description">${weaponDescription(weapon)}</p>
      ${statBarsHtml(weapon)}
      ${spreadVisualHtml(weapon)}
      <div class="ui231-exact-stats">
...
export class MainMenu {
  ...
  bindEvents() {
    this.root.addEventListener('click', (event) => {
      ...
      // Historical compatibility only. 2.3.1 Weapon Info is a complete
      // all-eight-weapon catalog and does not require selecting one weapon.
  ...
  renderWeaponInfo() {
    ...
    layout.classList.add('ui231-catalog-layout');
    layout.innerHTML = `
      <div class="ui231-weapon-catalog" data-weapon-info-catalog>
