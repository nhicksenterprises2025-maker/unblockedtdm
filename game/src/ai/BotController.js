...
export class BotController {
  ...
  update(dt, { camera, enemies = [], teammates = [], map }) {
    ...
    if (!hasLOS) {
      const laneY = (this.seed % 2 === 0 ? 6.5 : 15.5) * TILE_SIZE;
      const centerX = 16 * TILE_SIZE;
      const needsCenterRoute = Math.abs(this.player.x - centerX) > TILE_SIZE * 1.5 || Math.abs(this.player.y - laneY) > TILE_SIZE * 0.8;
      const waypoint = needsCenterRoute ? { x: centerX, y: laneY } : this.target;
      const route = normalize(waypoint.x - this.player.x, waypoint.y - this.player.y);
      moveX = route.x;
      moveY = route.y;
    } else if (distanceTiles > preferred * 1.22) {
      moveX = toward.x;
      moveY = toward.y;
