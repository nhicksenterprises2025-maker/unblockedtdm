...
assert.equal(MAP_02.cols, 38, 'The 2.6 Foundry overhaul must use its widened 38-column competitive footprint.');
assert.equal(MAP_02.rows, 24, 'The 2.6 Foundry overhaul must use its expanded 24-row competitive footprint.');
assert.equal(MAP_02.tileSize, MAP_01.tileSize, 'The Foundry expansion must preserve the proven world-unit scale.');
const foundryAreaRatio = (MAP_02.cols * MAP_02.rows) / (MAP_01.cols * MAP_01.rows);
assert.ok(foundryAreaRatio >= 1.28 && foundryAreaRatio <= 1.32, `Foundry Zero must remain approximately 1.3x Training Complex area (received ${foundryAreaRatio}).`);
...
assert.equal(map.width, MAP_02.cols * MAP_02.tileSize, 'TileMap must adopt Foundry Zero\'s active width instead of retaining the Casual bounds.');
assert.equal(map.height, MAP_02.rows * MAP_02.tileSize, 'TileMap must adopt Foundry Zero\'s active height instead of retaining the Casual bounds.');
